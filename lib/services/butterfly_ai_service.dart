...
class ButterflyAIService {
  ...
  Future<void> initInterpreter() async {
    try {
      interpreter = await Interpreter.fromAsset('assets/__2024-10-07_01_46.tflite');
    } catch (e) {
      devtools.log("Error loading model: $e");
    }
  }

  Future<Map<String, dynamic>> recognizeButterfly(File imageFile) async {
    ...
    img.Image resizedImage = img.copyResize(imageDecoded, width: inputShape[1], height: inputShape[2]);
    ...
    return {
      ...
      'positionModelOutput': maxIndex + 1,
    };
  }
